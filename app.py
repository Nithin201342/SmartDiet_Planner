...
@login_required
def get_prediction():
    profile = db.profiles.find_one({"user_id": current_user.id}) or {}
...
def save_workout():
    ...
    db.saved_workouts.insert_one(saved_workout)
...
def save_prediction():
    ...
    db.saved_plans.insert_one(saved_plan)
...
@login_required
def saved_plans():
    user_saved_diets = list(db.saved_plans.find({"user_id": current_user.id}).sort("saved_at", -1))
    user_saved_workouts = list(db.saved_workouts.find({"user_id": current_user.id}).sort("saved_at", -1))
...
def delete_plan(plan_id):
    result = db.saved_plans.delete_one({"_id": ObjectId(plan_id), "user_id": current_user.id})
...
def delete_workout(workout_id):
    result = db.saved_workouts.delete_one({"_id": ObjectId(workout_id), "user_id": current_user.id})
    if result.deleted_count == 1:
