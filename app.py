...
# ---------------------------------------------------------------------------
# Encoding maps for categorical profile fields.
#
# IMPORTANT: These must match EXACTLY the encoding used when the model was
# trained (e.g. the same mapping/order used for LabelEncoder / manual mapping
# in your training notebook/script). If you're not sure, check the training
# code for how "gender", "chronic_disease", "cholesterol_level", and
# "blood_sugar_level" were converted to numbers before fitting the scaler
# and model, and update the dicts below to match exactly.
# ---------------------------------------------------------------------------
...
def to_float(value, mapping=None, default=0.0):
    """
    Safely convert a profile field to float.
    - If a mapping is given and the value is a string, look it up
      (case-insensitive), falling back to `default` if not found.
    - Otherwise try a plain float conversion, falling back to `default`
      on failure (empty string, None, unexpected text, etc.).
    """
    if value is None:
    ...
    if mapping is not None and isinstance(value, str):
        ...
        # value wasn't in the map (e.g. blank or unexpected option) -> default
        return float(default)
...
def predict():
    ...
    try:
        # build features (categorical fields go through their encoding map)
        features = [
            to_float(profile.get("age")),
            to_float(profile.get("gender"), GENDER_MAP),
            to_float(profile.get("height_cm")),
            to_float(profile.get("weight_kg")),
            to_float(profile.get("bmi")),
            to_float(profile.get("chronic_disease"), YES_NO_MAP),
            to_float(profile.get("blood_pressure_systolic")),
            to_float(profile.get("blood_pressure_diastolic")),
            to_float(profile.get("cholesterol_level"), LEVEL_MAP),
            to_float(profile.get("blood_sugar_level"), LEVEL_MAP),
            to_float(profile.get("sleep_hours")),
        ]
        ...
        # Log the full traceback to Render logs so the real cause is visible,
        # instead of silently failing with a generic message.
        app.logger.exception("Prediction failed for user %s", current_user.id)
